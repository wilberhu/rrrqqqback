...
def save_result(result_dict, show_windows=True, savefile=None):
    import os
    import numpy as np
    ...
    fig_data = {
        u"Total Returns": "{0:.3%}".format(summary["total_returns"]),
        u"Annual Returns": "{0:.3%}".format(summary["annualized_returns"]),
        u"Benchmark Returns": "{0:.3%}".format(summary["benchmark_total_returns"]),
        u"Benchmark Annual": "{0:.3%}".format(summary["benchmark_annualized_returns"]),
        u"Alpha": "{0:.4}".format(summary["alpha"]),
        u"Beta": "{0:.4}".format(summary["beta"]),
        u"Sharpe": "{0:.4}".format(summary["sharpe"]),
        u"Sortino": "{0:.4}".format(summary["sortino"]),
        u"Information Ratio": "{0:.4}".format(summary["information_ratio"]),
        u"Volatility": "{0:.4}".format(summary["volatility"]),
        u"MaxDrawdown": "{0:.3}".format(summary["max_drawdown"]),
        u"Tracking Error": "{0:.4}".format(summary["tracking_error"]),
        u"Downside Risk": "{0:.4}".format(summary["downside_risk"])
    }
    ...
    dic = {}
    for i in fig_data:
        dic[i.replace(" ", "_")] = fig_data[i]

    data = json.dumps(dic, ensure_ascii=False, indent=4)
    f = open(savefile.split(".jpg")[0] + ".json", 'w')
    f.write(data)
    f.close()
    ...
    ##############################################################################
    # 累计收益
    df_portfolio = pd.DataFrame(np.round(portfolio["unit_net_value"].values - 1.0 ,decimals=5),
                                index=portfolio["unit_net_value"].index,
                                columns=["portfolio"])
    df_benchmark_portfolio = pd.DataFrame(np.round(benchmark_portfolio["unit_net_value"].values - 1.0 ,decimals=5),
                                index=benchmark_portfolio["unit_net_value"].index,
                                columns=["benchmark_portfolio"])

    # 成交记录
    date = []
    time = []
    for i in result_dict["trades"].index:
        date_time = i.split(" ")
        date.append(pd.to_datetime(date_time[0]))
        time.append(date_time[1])

    trades = result_dict["trades"]["last_price"]*result_dict["trades"]["last_quantity"]

    df_trade = pd.DataFrame({"date" :date,
                               "time": time,
                               "amount": trades.values,
                               "side": result_dict["trades"]["side"].values,
                               "symbol": result_dict["trades"]["symbol"].values
                               }).set_index("date")

    # 每日收益
    total_value = np.hstack((result_dict["summary"]["STOCK"], portfolio["total_value"].values))
    daily_returns = np.around(total_value[1:] - total_value[:-1], decimals=2)
    df_daily_returns = pd.DataFrame(daily_returns,
                                index=portfolio.index,
                                columns=["daily_returns"])

    # print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    # print(df_portfolio.shape, df_benchmark_portfolio.shape, df_trade.shape, df_daily_returns.shape)

    # 每日收益时间列表
    daily_returns_date = []
    for i in portfolio.index:
        date_time = str(i).split(" ")
        daily_returns_date.append(pd.to_datetime(date_time[0]))

    symbol_list= list(set(df_trade['symbol'].values.tolist()))

    matrix = [["" for i in range(len(symbol_list))] for j in range(len(daily_returns_date))]
    df_trade_matrix = pd.DataFrame(matrix, columns=symbol_list, index=daily_returns_date)
    df_side_matrix = pd.DataFrame(matrix, columns=symbol_list, index=daily_returns_date)

    for i in range(len(df_trade.index)):
        df_trade_matrix[df_trade["symbol"].iloc[i]].loc[df_trade.index[i]] = np.round(df_trade["amount"].iloc[i])
        df_side_matrix[df_trade["symbol"].iloc[i]].loc[df_trade.index[i]] = df_trade["side"].iloc[i]
    df_trade_matrix["date"] =  df_trade_matrix.index
    df_side_matrix["date"] = df_side_matrix.index

    # result_dict["trades"].to_csv(savefile.split(".jpg")[0] + "_trades.csv", index=False)
    df_trade_matrix.to_csv(savefile.split(".jpg")[0] + "_trade.csv", index=False)
    df_side_matrix.to_csv(savefile.split(".jpg")[0] + "_side.csv", index=False)

    # print("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    # print(result_dict)

    res = pd.concat([df_portfolio, df_benchmark_portfolio, df_daily_returns],
                    axis=1, join='outer', join_axes=None, ignore_index=False,
                    keys=None, levels=None, names=None, verify_integrity=False, copy=True)

    res["date"] = res.index
    res.to_csv(savefile.split(".jpg")[0] + ".csv", index=False)
